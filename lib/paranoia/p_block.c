/*
  Copyright (C) 2004, 2005, 2007, 2008, 2024 Rocky Bernstein <rocky@gnu.org>
...
void *get_elem(linked_element *e) { return (e->ptr); }
...
static c_block_t *i_cblock_constructor(cdrom_paranoia_t *p) {
...
static v_fragment_t *i_vfragment_constructor(void) {
  v_fragment_t *ret = calloc(1, sizeof(v_fragment_t));
  return (ret);
}

static void i_v_fragment_destructor(v_fragment_t *v) { free(v); }

v_fragment_t *new_v_fragment(cdrom_paranoia_t *p, c_block_t *one,
...
void free_v_fragment(v_fragment_t *v) { free_elem(v->e, 1); }
...
void c_set(c_block_t *v, long begin) { v->begin = begin; }
...
cdrom_paranoia_t *paranoia_init(cdrom_drive_t *d) {
  ...
  p->cache =
      new_list((void *)&i_cblock_constructor, (void *)&i_cblock_destructor);
